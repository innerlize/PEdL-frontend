...
export const VideosFilesField: React.FC = () => {
	const { values, setFieldValue } = useFormikContext<ProjectFormFieldsValues>();

	const fileTypes = ['video/mp4', 'video/avi', 'video/mkv', 'video/webm'];
	...
	const handleFileError = (errorCode: string) => {
		switch (errorCode) {
			case 'file-too-large':
				toast.error('File too large! Remember, 50 MB max.', {
					icon: () => <IoIosWarning />
				});
				break;
			...
			<div
				...
				<div className='flex justify-center'>
					<div
						...
						<GoVideo className='block relative size-[90px]' />
